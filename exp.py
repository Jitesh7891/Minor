...
def load_ml100k(path='d:/Minor/ml-100k/ml-100k'):
    # load only first 1000 ratings to speed up
      ...
      for lr in grid['lr']:
        for reg in grid['reg']:
          for ne in grid['n_epochs']:
            m = AttributeFusedSVD(nf, lr, reg, ne)
            m.fit(R, U_attr, I_attr)
            P = m.full_prediction()
            cur = calculate_rmse(P, R)
            if cur < best_rmse:
                best_rmse, best_params = cur, (nf, lr, reg, ne)
    return best_params, best_rmse

if __name__ == '__main__':
    # 1) load
    R, U_attr, I_attr = load_ml100k('d:/Minor/ml-100k/ml-100k')

    # 2) tune & report global best
    best_params, best_rmse = hyperparameter_tuning(R, U_attr, I_attr)
    print("Best Hyperparameters:", best_params)
    print(f"Global best RMSE: {best_rmse:.6f}\n")
    ...
    # 4) per-user RMSE (all users with ≥1 rating)
    valid = [u for u in range(R.shape[0]) if np.count_nonzero(R[u])>0]
    records = []
    for u in valid:
        u_rmse = calculate_rmse(P[u], R[u])
        records.append({'user_id': u+1, 'rmse': u_rmse})

    df = pd.DataFrame(records).sort_values('user_id')
    avg_rmse = df['rmse'].mean()
    print(f"Average per-user RMSE: {avg_rmse:.6f}\n")

    # 5) plot zig-zag line
    plt.figure(figsize=(12,6))
    plt.plot(df['user_id'], df['rmse'], marker='o', linestyle='-')
    plt.axhline(avg_rmse, color='red', linestyle='--',
                label=f'Average RMSE = {avg_rmse:.4f}')
    plt.xlabel('User ID')
    plt.ylabel('RMSE')
    plt.title('Per-user RMSE (Best Hyperparameters)')
    plt.legend()
    plt.tight_layout()
    plt.show()
